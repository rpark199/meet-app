...
export const getEvents = async () => {
	...
	if (token) {
		removeQuery();
		const url =
			'https://v9f320if2i.execute-api.eu-central-1.amazonaws.com/dev/api/get-events' +
			'/' +
			token;
	...
	if (!accessToken || tokenCheck.error) {
		...
		if (!code) {
			const response = await fetch(
				...
				'https://v9f320if2i.execute-api.eu-central-1.amazonaws.com/dev/api/get-auth-url'
			);
			const result = await response.json();
